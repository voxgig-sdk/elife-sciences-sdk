...
[MCP](https://modelcontextprotocol.io) server exposing the ElifeSciences SDK as
two agent tools — `elife-sciences_list` and `elife-sciences_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elife-sciences-mcp)
make build

# 2. Provide credentials via the environment
export ELIFE_SCIENCES_APIKEY=sk_live_xxx
...
```jsonc
// elife-sciences_list: first page of records
{ "entity": "annotation" }
{ "entity": "annotation", "query": { } }

// elife-sciences_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elife-sciences-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user elife-sciences \
     -- "$PWD"/dist/*/elife-sciences-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `elife-sciences_list` and `elife-sciences_load` tools now appear
   in new sessions. Ask the agent to *"list annotation using elife-sciences"*
   and it calls `elife-sciences_list` with `{"entity":"annotation"}`.
...
```sh
export ELIFE_SCIENCES_APIKEY=sk_live_xxx            # API key
export ELIFE_SCIENCES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `elife-sciences_list` tool
...
### Call the `elife-sciences_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `elife-sciences_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `elife-sciences_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ELIFE_SCIENCES_APIKEY` | API key sent with every request. |
| `ELIFE_SCIENCES_BASE` | Optional override of the API base URL. |
