...
func main() {
	...
	// Configure from the environment: ELIFE_SCIENCES_APIKEY carries the API key and
	// ELIFE_SCIENCES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ELIFE_SCIENCES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ELIFE_SCIENCES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewElifeSciencesSDK(opts)
