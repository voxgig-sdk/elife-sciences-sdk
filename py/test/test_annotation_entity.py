    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ELIFE_SCIENCES_TEST_ANNOTATION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        annotation_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.annotation")))
        annotation_ref01_data = None
        if len(annotation_ref01_data_raw) > 0:
...
def _annotation_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "ELIFE_SCIENCES_TEST_ANNOTATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ELIFE_SCIENCES_TEST_ANNOTATION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ELIFE_SCIENCES_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("ELIFE_SCIENCES_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ELIFE_SCIENCES_TEST_EXPLAIN") == "TRUE",
        "live": _live,
