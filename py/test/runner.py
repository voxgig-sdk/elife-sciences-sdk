...
class ElifeSciencesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ElifeSciencesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ElifeSciencesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ElifeSciencesTestRunner.getenv("ELIFE_SCIENCES_TEST_LIVE")
        override = ElifeSciencesTestRunner.getenv("ELIFE_SCIENCES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ElifeSciencesTestRunner.getenv(key)
        ...
        explain = ElifeSciencesTestRunner.getenv("ELIFE_SCIENCES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ElifeSciencesTestRunner._test_control is not None:
            return ElifeSciencesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ElifeSciencesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ElifeSciencesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ElifeSciencesTestRunner.load_test_control()
...
def load_env_local():
    ElifeSciencesTestRunner.load_env_local()


def env_override(m):
    return ElifeSciencesTestRunner.env_override(m)


def entity_data(v):
    return ElifeSciencesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ElifeSciencesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ElifeSciencesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ElifeSciencesTestRunner.load_test_control()


def live_client_options():
    return ElifeSciencesTestRunner.live_client_options()


def live_delay_ms():
    return ElifeSciencesTestRunner.live_delay_ms()
