...
class TestSubjectDirect:

    def test_should_direct_load_subject(self):
        setup = _subject_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-subject", "live" if setup["live"] else "unit")
        if _skip:
...
def _subject_direct_setup(mockres):
