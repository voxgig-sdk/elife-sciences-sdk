...
from elifesciences_sdk.elifesciences_types import (
    Subject,
    SubjectLoadMatch,
)
...
class SubjectEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SubjectEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Subject:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Subject().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
