...
module ElifeSciencesOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if ElifeSciencesOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = ElifeSciencesOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| ElifeSciencesOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || ElifeSciencesOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = ElifeSciencesSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = ElifeSciencesError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { ElifeSciencesOmni.errify(err) }
