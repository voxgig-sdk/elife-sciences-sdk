...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ElifeSciencesOmni.make_runner(TEST_JSON_FILE, ElifeSciencesSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    ElifeSciencesError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ElifeSciencesOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = ElifeSciencesOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def auth_credential(client)
    utility = client.get_utility
    ctx = make_test_ctx(client, utility, nil)
    ctx.spec = ElifeSciencesSpec.new({ "headers" => {}, "query" => {}, "step" => "s" })
