# ElifeSciences SDK utility: fetcher
...
module ElifeSciencesUtilities
  ...
  DefaultHttpFetch = ->(fullurl, fetchdef) {
    ...
    begin
      ...
      request['User-Agent'] = 'Mozilla/5.0 (compatible; ElifeSciencesSDK/1.0)' unless has_ua
      request.body = body_str if body_str.is_a?(String)

      http = ElifeSciencesUtilities.http_checkout(key, uri, proxy)
      begin
        # Net::HTTP retries idempotent requests; an EOF can follow a committed POST.
        resp = http.request(request)
      rescue StandardError
        ElifeSciencesUtilities.http_discard(http)
        raise
      end
      ElifeSciencesUtilities.http_checkin(key, http)
