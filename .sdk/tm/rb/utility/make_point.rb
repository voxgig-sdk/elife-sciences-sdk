...
module ElifeSciencesUtilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(ElifeSciencesError)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      op.points.each do |p|
        point = p
        select_def = ElifeSciencesHelpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = ElifeSciencesHelpers.to_map(VoxgigStruct.getprop(point, "select"))
