...
module ElifeSciencesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ElifeSciencesResponse.new({})
    ...
    if fetch_err
      ...
      response = ElifeSciencesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ElifeSciencesResponse.new(fetched)
    else
