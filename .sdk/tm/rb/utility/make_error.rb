...
module ElifeSciencesUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ElifeSciencesContext.new({}, nil)
    end
    op = ctx.op || ElifeSciencesOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ElifeSciencesResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ElifeSciencesError) ? err.msg : err.to_s
    ...
    sdk_err = ElifeSciencesError.new("", msg, ctx)
    sdk_err.result = ctx.utility.clean.call(ctx, result)
    sdk_err.spec = ctx.utility.clean.call(ctx, spec)
    sdk_err.code = err.code if err.is_a?(ElifeSciencesError)
