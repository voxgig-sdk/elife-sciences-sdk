# ElifeSciences SDK Annotation entity
...
class AnnotationEntity
  ...
  def make
    opts = @_entopts.dup
    AnnotationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ElifeSciencesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Annotation, Hash] the current Annotation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ElifeSciencesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Annotation fields)
  def match_get
  ...
  #
  # @param reqmatch [AnnotationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Annotation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Annotation, Hash] the loaded Annotation; raises ElifeSciencesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ElifeSciencesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
