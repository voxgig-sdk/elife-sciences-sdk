...
The Ruby SDK for the ElifeSciences API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Annotation(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.ElifeSciences(nil).load(
  { "id" => "test01" }, nil
)
