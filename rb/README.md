# ElifeSciences Ruby SDK



The Ruby SDK for the ElifeSciences API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Annotation` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
raise a `ElifeSciencesError` (a `StandardError` subclass), so wrap
...
search = client.Search.load({ "for" => "for" })
```
...
```
rb/
├── ElifeSciences_sdk.rb       -- Main SDK module
...
The main module (`ElifeSciences_sdk`) exports the SDK class
