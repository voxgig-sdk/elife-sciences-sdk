...
class AnnotationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_ANNOTATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    annotation_ref01_data = nil
    if annotation_ref01_data_raw.length > 0
    ...
    annotation_ref01_data_dt0_loaded, err = annotation_ref01_ent.load(annotation_ref01_match_dt0, nil)
    assert_nil err
    assert !annotation_ref01_data_dt0_loaded.nil?
...
def annotation_basic_setup(extra)
  ...
  entid_env_raw = ENV["ELIFESCIENCES_TEST_ANNOTATION_ENTID"]
