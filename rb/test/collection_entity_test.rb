...
class CollectionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_COLLECTION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    collection_ref01_data_dt0_loaded, err = collection_ref01_ent.load(collection_ref01_match_dt0, nil)
    assert_nil err
...
def collection_basic_setup(extra)
  ...
  entid_env_raw = ENV["ELIFESCIENCES_TEST_COLLECTION_ENTID"]
