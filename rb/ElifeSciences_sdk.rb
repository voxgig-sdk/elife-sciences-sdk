...
class ElifeSciencesSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ElifeSciencesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ElifeSciencesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ElifeSciencesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ElifeSciencesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = ElifeSciencesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ElifeSciencesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ElifeSciencesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ElifeSciencesError.new(
        "graphql_error", "ElifeSciencesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ElifeSciencesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
