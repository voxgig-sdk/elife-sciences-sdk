...
class ElifeSciencesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ElifeSciencesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ElifeSciencesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ElifeSciencesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ElifeSciencesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ElifeSciencesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ElifeSciencesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ElifeSciencesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ElifeSciencesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ElifeSciencesError($op . "_allow",
                "ElifeSciencesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ElifeSciencesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ElifeSciencesError("graphql_error",
                "ElifeSciencesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Subject($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_subject === null) {
                $this->_subject = new SubjectEntity($this, null);
            }
            return $this->_subject;
        }
        return new SubjectEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new ElifeSciencesSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
