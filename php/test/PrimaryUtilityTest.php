...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new ElifeSciencesContext($ctxmap, null);
    ...
    private static function fixctx(ElifeSciencesContext $ctx, ElifeSciencesSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ElifeSciencesError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ElifeSciencesError($code, $msg);
    }
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ElifeSciencesSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new ElifeSciencesOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ElifeSciencesSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ElifeSciencesError) ? $result->err->msg : (string)$result->err;
