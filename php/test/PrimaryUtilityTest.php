...
// ElifeSciences SDK primary utility test
//
...
//   ElifeSciencesOmni::unwrap, which throws the err so omni can match it
...
//   builds the typed context with ElifeSciencesOmni::ctx_from_map, runs the
//   utility, and writes the observable state back with
//   ElifeSciencesOmni::sync_ctx BEFORE throwing any error - that map is
...
class PrimaryUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ElifeSciencesOmni::makeRunner(
                self::TEST_JSON_FILE, ElifeSciencesSDK::test(null, null));
    ...
    private static function client(): ElifeSciencesSDK
    {
    ...
    private static function make_test_ctx(
        ElifeSciencesSDK $client,
        ElifeSciencesUtility $utility,
        ?array $overrides = null
    ): ElifeSciencesContext {
        $ctxmap = [
            'opname' => 'load',
            'client' => $client,
            'utility' => $utility,
    ...
    private static function auth_credential(
        ...
        $ctx->spec = new ElifeSciencesSpec(['headers' => [], 'query' => []]);
    ...
    public function test_done_basic(): void
    {
        ...
        $this->runsection('done', function (array $ctxmap) use ($client, $utility) {
            $ctx = ElifeSciencesOmni::ctx_from_map($ctxmap, $client, $utility);
    ...
    public function test_make_error_basic(): void
    {
        ...
        $this->runsection('makeError', function ($ctxmap = null, $errmap = null) use ($client, $utility) {
            $ctx = ElifeSciencesOmni::ctx_from_map(
                is_array($ctxmap) ? $ctxmap : [], $client, $utility);
            $err = ElifeSciencesOmni::err_from_map(is_array($errmap) ? $errmap : null);
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new ElifeSciencesResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new ElifeSciencesBaseFeature();
    ...
    {
        $hook_client = ElifeSciencesSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends ElifeSciencesBaseFeature {
            public $hook_fn = null;
            public function TestHook(ElifeSciencesContext $ctx): void
            {
    ...
    {
        $init_client = ElifeSciencesSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends ElifeSciencesBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new ElifeSciencesSDK([
    ...
    {
        $blocked_client = new ElifeSciencesSDK([
            'base' => 'http://localhost:8080',
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ElifeSciencesSpec([
            'base' => 'http://localhost:8080',
            'prefix' => '/api',
            'path' => 'items/{id}',
            'suffix' => '',
    ...
    public function test_make_request_basic(): void
    {
        ...
        $this->runsection('makeRequest', function (array $ctxmap) use ($client, $utility) {
            ...
            ElifeSciencesOmni::sync_ctx($ctxmap, $ctx);
            if ($err !== null) {
                throw $err;
            }
            return null;
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ElifeSciencesSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ElifeSciencesSDK::test(null, is_array($setup) ? $setup : null);
        $spec_utility = $spec_client->get_utility();
        $utility = self::client()->get_utility();

        $this->runsection('makeSpec', function (array $ctxmap) use ($spec_client, $spec_utility, $utility) {
            $ctx = ElifeSciencesOmni::ctx_from_map($ctxmap, $spec_client, $spec_utility);
    ...
    public function test_make_url_basic(): void
    {
        ...
        $this->runsection('makeUrl', function (array $ctxmap) use ($client, $utility) {
            ...
            return ElifeSciencesOmni::unwrap(($utility->make_url)($ctx));
        });
    }
    ...
    public function test_operator_basic(): void
    {
        $this->runsection('operator', function ($vin = null) {
            ...
            $op = new ElifeSciencesOperation($vin);
            return [
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ElifeSciencesSDK::test(null, is_array($setup) ? $setup : null);
        ...
        $this->runsection('prepareAuth', function (array $ctxmap) use ($auth_client, $auth_utility, $utility) {
            $ctx = ElifeSciencesOmni::ctx_from_map($ctxmap, $auth_client, $auth_utility);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runsection('resultBasic', function (array $ctxmap) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ElifeSciencesError)
