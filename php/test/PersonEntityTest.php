...
class PersonEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_PERSON_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function person_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ELIFESCIENCES_TEST_PERSON_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ELIFESCIENCES_TEST_PERSON_ENTID"]);
