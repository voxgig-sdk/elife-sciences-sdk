...
// ElifeSciences SDK feature test
//
...
class FtCtrl extends ElifeSciencesControl
{
...
class FtClient
{
    ...
    private ElifeSciencesUtility $utility;

    public function __construct(ElifeSciencesUtility $utility, array $options)
    {
    ...
    public function get_utility(): ElifeSciencesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ElifeSciencesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ElifeSciencesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ElifeSciencesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ElifeSciencesConfig::make_feature($name);
    ...
    private static function buildUrl(ElifeSciencesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ElifeSciencesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ElifeSciencesResponse($fetched);
        }
    ...
    private function fail(ElifeSciencesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ElifeSciencesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
