...
class SubjectEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Subject(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = subject_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "subject." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $subject_ref01_data = null;
        if (count($subject_ref01_data_raw) > 0) {
...
function subject_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/subject/SubjectTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["subject01", "subject02", "subject03"] as $k) {
