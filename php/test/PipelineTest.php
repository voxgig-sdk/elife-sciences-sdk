...
// ElifeSciences SDK pipeline test
//
...
{
    private static function utility(): ElifeSciencesUtility
    {
        return new ElifeSciencesUtility();
    }
    ...
    private static function ctx(array $over = []): ElifeSciencesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ElifeSciencesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ElifeSciencesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ElifeSciencesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ElifeSciencesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ElifeSciencesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ElifeSciencesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ElifeSciencesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ElifeSciencesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ElifeSciencesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ElifeSciencesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ElifeSciencesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ElifeSciencesResponse([]),
            'result' => new ElifeSciencesResult([]),
        ]);
        [, $err] = ElifeSciencesMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec([]),
            'response' => null,
            'result' => new ElifeSciencesResult([]),
        ]);
        [, $err] = ElifeSciencesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec(['step' => 's']),
            'response' => new ElifeSciencesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ElifeSciencesResult([]),
        ]);
        [, $err] = ElifeSciencesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec(['step' => 's']),
            'response' => new ElifeSciencesResponse(self::resp(200, ['v' => 1])),
            'result' => new ElifeSciencesResult([]),
        ]);
        [, $err] = ElifeSciencesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec(['step' => 's']),
            'response' => new ElifeSciencesResponse(self::resp(200, ['v' => 2])),
            'result' => new ElifeSciencesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ElifeSciencesMakeResponse::call($ctx);
    ...
    {
        $preset = new ElifeSciencesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec([]),
            'response' => new ElifeSciencesResponse([]),
            'result' => new ElifeSciencesResult([]),
        ...
        [$got, $err] = ElifeSciencesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ElifeSciencesResult([])]);
        [, $err] = ElifeSciencesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ElifeSciencesSpec([]), 'result' => null]);
        [, $err] = ElifeSciencesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ElifeSciencesSpec(['step' => 's']),
            'result' => new ElifeSciencesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ElifeSciencesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ElifeSciencesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ElifeSciencesSpec(['step' => 's']),
            'result' => new ElifeSciencesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ElifeSciencesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ElifeSciencesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ElifeSciencesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ElifeSciencesSpec([]),
            'result' => new ElifeSciencesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ElifeSciencesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ElifeSciencesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ElifeSciencesError('boom', 'boom');
        $utility->fetcher = function (ElifeSciencesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ElifeSciencesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ElifeSciencesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ElifeSciencesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ElifeSciencesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ElifeSciencesResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ElifeSciencesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ElifeSciencesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ElifeSciencesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ElifeSciencesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ElifeSciencesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ElifeSciencesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ElifeSciencesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ElifeSciencesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ElifeSciencesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ElifeSciencesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ElifeSciencesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ElifeSciencesResult(['ok' => false])]);
        $this->expectException(ElifeSciencesError::class);
        ElifeSciencesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ElifeSciencesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ElifeSciencesMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ElifeSciencesMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ElifeSciencesBaseFeature();
        $b = new ElifeSciencesBaseFeature();
        ElifeSciencesFeatureAdd::call($ctx, $a);
        ElifeSciencesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ElifeSciencesBaseFeature
    {
        $f = new ElifeSciencesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ElifeSciencesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?ElifeSciencesSpec $spec): ElifeSciencesContext
    {
    ...
    private static function auth_bags(): ElifeSciencesSpec
    {
        return new ElifeSciencesSpec(['headers' => [], 'query' => []]);
    }
    ...
    private static function auth_probe(array $options): ?array
    {
        ...
        ElifeSciencesPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ElifeSciencesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ElifeSciencesResponse(['headers' => null]),
            'result' => new ElifeSciencesResult([]),
        ]);
        ElifeSciencesResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ElifeSciencesResultBody::call($ctx);
