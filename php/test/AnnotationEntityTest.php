...
class AnnotationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_ANNOTATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $annotation_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.annotation")));
        $annotation_ref01_data = null;
        if (count($annotation_ref01_data_raw) > 0) {
            $annotation_ref01_data = Helpers::to_map($annotation_ref01_data_raw[0][1]);
        }

        // LOAD
        $annotation_ref01_ent = $client->Annotation(null);
        $annotation_ref01_match_dt0 = [];
        [$annotation_ref01_data_dt0_loaded, $err] = $annotation_ref01_ent->load($annotation_ref01_match_dt0, null);
        $this->assertNull($err);
        $this->assertNotNull($annotation_ref01_data_dt0_loaded);
...
function annotation_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["annotation01", "annotation02", "annotation03"] as $k) {
    ...
    $entid_env_raw = getenv("ELIFESCIENCES_TEST_ANNOTATION_ENTID");
