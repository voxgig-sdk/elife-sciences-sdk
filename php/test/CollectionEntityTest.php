...
class CollectionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_COLLECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$collection_ref01_data_dt0_loaded, $err] = $collection_ref01_ent->load($collection_ref01_match_dt0, null);
        $this->assertNull($err);
...
function collection_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("ELIFESCIENCES_TEST_COLLECTION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ELIFESCIENCES_TEST_COLLECTION_ENTID"]);
