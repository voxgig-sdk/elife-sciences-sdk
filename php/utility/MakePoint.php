...
// ElifeSciences SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class ElifeSciencesMakePoint
{
    public static function call(ElifeSciencesContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof ElifeSciencesError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = ElifeSciencesHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = ElifeSciencesHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
