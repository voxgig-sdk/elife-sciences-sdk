...
// ElifeSciences SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ElifeSciencesTestFeature extends ElifeSciencesBaseFeature
{
    ...
    public function init(ElifeSciencesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ElifeSciencesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ElifeSciencesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ElifeSciencesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ElifeSciencesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ElifeSciencesParam::call($ctx, $k);
