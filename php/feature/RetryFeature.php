...
// ElifeSciences SDK retry feature
...
class ElifeSciencesRetryFeature extends ElifeSciencesBaseFeature
{
    ...
    public function init(ElifeSciencesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ElifeSciencesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ElifeSciencesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ElifeSciencesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
