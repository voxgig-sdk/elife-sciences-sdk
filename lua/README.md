# ElifeSciences Lua SDK



The Lua SDK for the ElifeSciences API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Annotation()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── elife-sciences_sdk.lua    -- Main SDK module
...
The main module (`elife-sciences_sdk`) exports the SDK constructor
