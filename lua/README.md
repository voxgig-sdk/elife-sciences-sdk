...
The Lua SDK for the ElifeSciences API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Annotation(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ElifeSciences(nil):load(
  { id = "test01" }, nil
)
