...
local SubjectEntity = {}
SubjectEntity.__index = SubjectEntity


function SubjectEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SubjectEntity)
  self._name = "subject"
  self._client = client
...
function SubjectEntity:get_name()
  return self._name
end


function SubjectEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SubjectEntity.new(self._client, opts)
end
...
function SubjectEntity:mark_deleted()
  self._deleted = true
end


function SubjectEntity:deleted()
  return true == self._deleted
end


function SubjectEntity:data_set(args)
  if args ~= nil then
...
function SubjectEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SubjectEntity:match_set(args)
  if args ~= nil then
...
function SubjectEntity:match_get()
...
function SubjectEntity:stream(action, args, callopts)
...
function SubjectEntity:_run_op(ctx, post_done)
  local utility = self._utility
