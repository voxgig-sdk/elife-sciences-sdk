...
//
// NewElifeSciencesSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ElifeSciencesSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewElifeSciencesSDK(opts)
}
...
func fcCandidates(client *sdk.ElifeSciencesSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ElifeSciencesSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ElifeSciencesSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.ElifeSciencesError
