...
import (
	...
	omni "github.com/voxgig-sdk/elife-sciences-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ElifeSciencesError); is {
				out := map[string]any{
					"name":    "ElifeSciencesError",
...
func omniCtx(arg any, client *sdk.ElifeSciencesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ElifeSciencesSDK); is {
			client = live
