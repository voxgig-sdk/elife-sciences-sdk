...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELIFE_SCIENCES_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELIFE_SCIENCES_TEST_SEARCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ELIFE_SCIENCES_TEST_SEARCH_ENTID"])
