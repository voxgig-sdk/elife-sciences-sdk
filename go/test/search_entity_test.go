...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELIFESCIENCES_TEST_SEARCH_ENTID")
	idmapOverridden := entidEnvRaw != "" && strings.HasPrefix(strings.TrimSpace(entidEnvRaw), "{")

	env := envOverride(map[string]any{
		"ELIFESCIENCES_TEST_SEARCH_ENTID": idmap,
		"ELIFESCIENCES_TEST_LIVE":      "FALSE",
		"ELIFESCIENCES_TEST_EXPLAIN":   "FALSE",
	})

	idmapResolved := core.ToMapAny(env["ELIFESCIENCES_TEST_SEARCH_ENTID"])
