...
func TestPersonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELIFESCIENCES_TEST_PERSON_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func personBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELIFESCIENCES_TEST_PERSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ELIFESCIENCES_TEST_PERSON_ENTID"])
