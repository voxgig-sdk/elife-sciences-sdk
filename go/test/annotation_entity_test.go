...
func TestAnnotationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ELIFE_SCIENCES_TEST_ANNOTATION_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		annotationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.annotation")))
...
func annotationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ELIFE_SCIENCES_TEST_ANNOTATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ELIFE_SCIENCES_TEST_ANNOTATION_ENTID"])
