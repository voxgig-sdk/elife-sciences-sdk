# ElifeSciences Golang SDK



The Golang SDK for the ElifeSciences API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.

It exposes the API as capitalised, semantic **Entities** — e.g. `client.Annotation(nil)` — each with the same small set of operations (`Load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
## Install
```bash
go get github.com/voxgig-sdk/elife-sciences-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/elife-sciences-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/elife-sciences-sdk/go=../elife-sciences-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *ElifeSciencesSDK
```
...
| `Subject` | `(data map[string]any) ElifeSciencesEntity` | Create a Subject entity instance. |

### Entity interface (ElifeSciencesEntity)

All entities implement the `ElifeSciencesEntity` interface.
...
```go
search, err := client.Search(nil).Load(map[string]any{"for": "for"}, nil)
if err != nil {
    panic(err)
}
...
Create an instance: `subject := client.Subject(nil)`
...
```
github.com/voxgig-sdk/elife-sciences-sdk/go/
├── elife-sciences.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/elife-sciences-sdk/go`) re-exports everything needed
