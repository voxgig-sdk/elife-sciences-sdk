...
`client.Annotation()` — each with a small set of operations (`load`)
...
```ts
try {
  const article = await client.Article().load({ id: "example_id" })
  console.log(article)
} catch (err) {
  console.error('load failed:', err)
}
...
```ts
const search = await client.Search().load({ for: 'for' })
```
