...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ELIFESCIENCES_TEST_LIVE ||
    'TRUE' === process.env.ELIFESCIENCES_TEST_OVERRIDE
  ) {
  ...
  m.ELIFESCIENCES_TEST_EXPLAIN = process.env.ELIFESCIENCES_TEST_EXPLAIN || m.ELIFESCIENCES_TEST_EXPLAIN
