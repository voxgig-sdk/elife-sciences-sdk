...
describe('SubjectDirect', async () => {
  ...
  test('direct-load-subject', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-subject', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ELIFE_SCIENCES_TEST_SUBJECT_ENTID']
