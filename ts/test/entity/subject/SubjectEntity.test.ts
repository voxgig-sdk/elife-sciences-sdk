...
describe('SubjectEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'subject.' + op, live)) return
    }
    ...
    const subject_ref01_data_dt0 = await subject_ref01_ent.load(subject_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['ELIFE_SCIENCES_TEST_SUBJECT_ENTID']
