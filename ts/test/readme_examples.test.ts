...
//     (\`new ElifeSciencesSDK(...)\` / \`ElifeSciencesSDK.test(...)\` become
//     \`ElifeSciencesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ElifeSciencesSDK'
...
const TEST_SEED = {"entity":{"annotation":{"test01":{"id":"test01"}},"article":{"test01":{"id":"test01"}},"collection":{"test01":{"id":"test01"}},"person":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}},"subject":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ElifeSciencesSDK(...)\` and any
// \`ElifeSciencesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ElifeSciencesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ElifeSciencesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
