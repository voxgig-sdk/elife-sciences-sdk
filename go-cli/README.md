...
AQL-driven command-line client **and** interactive REPL for the ElifeSciences
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/elife-sciences-cli)
make build

# 2. See usage (words, entities, env vars)
./elife-sciences-cli --help
...
./elife-sciences-cli load 1 annotation            # {id:1} shorthand
./elife-sciences-cli load '{id:1}' annotation       # explicit match map

# 5. Override the API base URL for a single call
ELIFE_SCIENCES_BASE=https://api.example.com ./elife-sciences-cli load 1 annotation

# 6. No arguments -> interactive REPL
./elife-sciences-cli
elife-sciences> load 1 annotation
elife-sciences> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/elife-sciences-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/elife-sciences-cli load 1 annotation
   ```
...
```sh
./elife-sciences-cli load 1 annotation          # scalar shorthand for {id:1}
./elife-sciences-cli load '{id:1}' annotation     # explicit match map
```
...
./elife-sciences-cli load 1 annotation
```
...
Run with no arguments to open a REPL (prompt `elife-sciences>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `annotation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/elife-sciences-cli`. |
...
(`./elife-sciences-cli <expr>`) and interactively (the REPL), and expressions compose the
